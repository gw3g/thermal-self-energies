...
int main() {
  ...
  HTL = 2; record_PI(2.1, 2.);
  /*HTL = 1;    eval_T(0.01, 2.); */
  /*HTL = 2;    eval_T(0.01, 2.); */
  /*HTL = 1;    eval_PI(0.01, 2.);  eval_disp(0.0001, 3.5);*/
  /*HTL = 2;    eval_PI(0.01, 2.);  eval_disp(0.3, .5);*/
...
void record_PI(double o_max, double q_max)
{

       if (HTL==1) sprintf(fname, "out/qo Pi, htl.csv"                                               );
  else if (HTL==2) sprintf(fname, "out/qo Pi, qed.csv"                                               );
  else if (HTL==3) sprintf(fname, "out/qo Pi, qcd.csv"                                               );
  ...
  fprintf(file,   "#\n"                                                                              );
  fprintf(file,   "# o/T, q/T, Re(Pi_L),  Im(Pi_L),  Re(Pi_T),  Im(Pi_T)\n"                          );

  double o,q;
  double *piL;   double *piT;

  for(int i=0; i<points; i++) {
    o =   1e-4 + (o_max)*( (double) i )/( (double) points );
  for(int j=0; j<points; j++) {
    q =   1e-4 + (q_max)*( (double) j )/( (double) points );
